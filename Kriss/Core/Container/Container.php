...
class Container implements ContainerInterface
{
    ...
    public function has($id) {
        return class_exists($id)||isset($this->container[$this->getId($id)]);
    }
    ...
    public function get($id) {
        if ($this->has($this->getId($id).'_instance')) {
            ...
            $instance = is_callable($get)?$get($this):$get;
    ...
    public function set($id, $rules = array()) {
        $this->rules[$id] = $rules;
        $this->container[$this->getId($id).'_class'] = $id;
        $id = $this->getId($id);
        foreach ($rules as $key => $rule) {
            ...
            case 'shared':
                ...
                break;
            default:
                trigger_error(__METHOD__.': '.$key.' rule is not defined');
                break;
            }
        }
        
        $this->container[$id] = function ($container) use ($id) {
